                ...
                return (
                  <div className="country-card w-full hover:scale-105 rounded-sm transition-transform duration-200 ease-in-out" key={i}>
                    ...
                    <div className="country-card-text py-5 px-6">
                      <Link to={linkPath}>
                        <h2 className="mb-5 cursor-pointer text-lg">{el.name["common"]}</h2>
                      </Link>
                      <div><span>Population : </span> {el.population}</div>
                      { el.region && <div><span>Region : </span> {el.region}</div> }
                      { el.capital && <div><span>Capital : </span> {el.capital}</div> }
                    </div>
                  </div>
                )
              })
