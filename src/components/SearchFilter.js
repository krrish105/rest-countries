export const SearchFilter = ({ region, setRegion, submitHandler }) => {
	return (
		<div className="search-filter mt-8 mb-12 flex flex-col sm:flex-row gap-4">
			{/* Country Search */}
			<form
				className="flex items-center py-4 px-6 w-full sm:w-fit"
				onSubmit={(e) => {
					e.preventDefault();
			...
			<fieldset className="region-selector flex flex-col w-[250px] relative">
				<div
					className="py-3 px-4 relative flex legend cursor-pointer"
					onClick={() =>
						document.querySelector(".dropdown").classList.toggle("active-drop")
					}
				>
